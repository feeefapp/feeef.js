...
test.group('feeef.js OAuth scope catalog', () => {
  test('member catalog includes integrations, templates, finance, inventory', ({
    assert,
  }) => {
    assert.includeMembers([...MEMBER_SCOPES], [
      'store.integrations',
      'store_templates',
      'template_components',
      'finance',
  ...
  test('OAUTH_SCOPES is member + platform, identity default is auth only', ({
    assert,
  }) => {
